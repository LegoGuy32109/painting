...
const blendColors = (currentColor: string, newColor: string, alpha: number) => {
  ...
  const blendChannel = (newChannel: number, currentChannel: number) =>
    Math.round(newChannel * alpha + currentChannel * (1 - alpha));
...
const makeMatrix = <T,>(length: number, value: T): T[][] =>
  Array.from({ length }, () => Array.from({ length }, () => value));
...
function App() {
          ...
          setStrokeData((currentStrokeData) => {
            setCanvasData((prev) => {
              setCanvasHistory((history) => {
                ...
                if (
                  history.length > 0 &&
                  prev.canvas.some((row, x) =>
                    row.some((cell, y) => cell !== history[0][x][y])
                  )
                ) {
  ...
  const drawBrush = (x: number, y: number) => {
    ...
    mapPlain.split("").forEach((char, index) => {
      if (char === BRUSH_CHARACTER || char === CURSOR_CHARACTER) {
        ...
        if (
          xCoord >= 0 && xCoord < GRID_SIZE && yCoord >= 0 && yCoord < GRID_SIZE
        ) {
  ...
  const overlayStrokeData = (
    canvasData: CanvasData,
    updatedStrokeData: boolean[][],
  ) => {
    ...
    for (let x = 0; x < GRID_SIZE; x++) {
      for (let y = 0; y < GRID_SIZE; y++) {
        ...
        const color = strokeHere
          ? blendColors(currentColor, brushColor, opacity)
          : currentColor;
        ctx.fillStyle = color;
