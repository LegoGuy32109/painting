 ...
 */
import { migrateDatabase, pendingMigrations } from "../src/server/migrations.ts";
import { openEnvironmentDatabase, parseDatabaseEnvironment } from "./database-environment.ts";

const db = await openEnvironmentDatabase(parseDatabaseEnvironment("Production"));
if (Deno.args.includes("--dry-run")) {
  ...
  console.log(applied.length ? `Applied: ${applied.join(", ")}` : "No pending production migrations.");
}
