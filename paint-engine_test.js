...
Deno.test("a stamp clips cells beyond the canvas edge", () => {
  ...
  const changes = applyStamp(pixels, 3, 3, { x: 0, y: 0 }, 4, red, new Set());
...
Deno.test("one drag never changes the same cell twice", () => {
  ...
  const first = applyStamp(pixels, 3, 3, { x: 1, y: 1 }, 2, blue, seen);
  const second = applyStamp(pixels, 3, 3, { x: 1, y: 1 }, 2, blue, seen);
...
Deno.test("a stamp writes row-major pixel indexes", () => {
  ...
  applyStamp(pixels, 4, 3, { x: 2, y: 1 }, 1, green, new Set());
